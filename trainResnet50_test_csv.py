...
from resnet50 import ResNet50_Flexible
...
if not os.path.exists('results/resnet50'):
    os.makedirs('results/resnet50')

model_id = len(os.listdir('results/resnet50')) + 1
save_path = f'results/resnet50/modelo{model_id}_test_csv'
...
for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(train_dataset)))):
    ...
    model = ResNet50_Flexible().to("cuda" if torch.cuda.is_available() else "cpu")

...
if not os.path.exists('results/efficientNetv2M'):
    os.makedirs('results/efficientNetv2M')

model_id = len(os.listdir('results/efficientNetv2M')) + 1
save_path = f'results/efficientNetv2M/modelo{model_id}_test_csv'
...
# Leer los IDs de test definidos externamente
import pandas as pd
test_ids_df = pd.read_csv('./dataset/test.csv')  # Asegúrate de que la ruta sea correcta
test_ids = test_ids_df['Code'].tolist()

# Filtrar índices del dataset usando los IDs
all_ids = list(descriptor_data['float'].keys())  # Este orden es el usado en final_data
test_indices = [i for i, cid in enumerate(all_ids) if cid in test_ids]
train_indices = [i for i in range(len(all_ids)) if i not in test_indices]

# Crear datasets usando los índices personalizados
from torch.utils.data import Subset

full_dataset = LD50Dataset()
...
for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(train_dataset)))):
    if fold == 0 or fold == 1 or fold == 2  or fold == 3:
        continue
    print(f"\n### Fold {fold + 1}/{k_folds} ###")
    ...
    model = EfficientNetV2MultiInput().to("cuda" if torch.cuda.is_available() else "cpu")

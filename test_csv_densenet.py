...
models = []
for i in range(1, 6):
    model = FlexibleDenseNet(growth_rate=32, num_classes=1).to(device)
    model.load_state_dict(torch.load(os.path.join(MODEL_DIR, f"fold{i}.pth"), map_location=device))
    model.eval()
    models.append(model)
...
output_path = os.path.join(MODEL_DIR, "predicciones_densenet121_test.csv")

...
if not os.path.exists('results/densenet121'):
    os.makedirs('results/densenet121')

model_id = len(os.listdir('results/densenet121')) + 1
save_path = f'results/densenet121/modelo{model_id}'
...
for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(train_dataset)))):
    ...
    model = FlexibleDenseNet(growth_rate=32, num_classes=1).to("cuda" if torch.cuda.is_available() else "cpu")

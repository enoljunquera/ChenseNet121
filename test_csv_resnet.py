#Este script toma los datos de test y ejecuta los fold correspondientes. Solo para modelos entrenados con test.csv como partición de test!!
import os
import torch
import pandas as pd
import numpy as np
...
# Parsear argumentos
parser = argparse.ArgumentParser(description="Evaluación de modelos ResNet50 en conjunto de test.")
...
df_all = pd.read_csv("./dataset/combined_dataset.csv")
...
models = []
for i in range(1, 6):
    model = ResNet50_Flexible().to(device)
...
df_result.to_csv(model_dir + "/predicciones_resnet50_test.csv", index=False)
print("Predicciones guardadas en "+model_dir+"/predicciones_resnet50_test.csv")
